...
"""Generate the bounded deterministic Nature's Aura visual gallery."""
...
def placements_tsv() -> bytes:
    lines = ["case_id\tlabel\tx\ty\tz\tblock_state\texpected\tnbt"]
    lines.extend(
        "\t".join(
            (
                placement.case_id,
...
def build_function() -> bytes:
    ...
    lines = [
        ...
        f"fill {minimum_x} {minimum_y} {minimum_z} "
        f"{maximum_x} {minimum_y} {maximum_z} minecraft:smooth_stone",
    ]
    for placement in cases.PLACEMENTS:
        lines.extend(
            (
...
def verify_function() -> bytes:
    ...
    lines.append(
        "tellraw @a {\"text\":\"Nature's Aura gallery checked\","
        "\"color\":\"green\"}"
    )
    return text_bytes("\n".join(lines))
...
def base_files() -> dict[Path, bytes]:
    ...
    teleport = " ".join(str(value) for value in cases.TELEPORT)
    return {
        Path("placements.tsv"): placements_tsv(),
        Path("datapack/pack.mcmeta"): json_bytes(
            {
                "pack": {
                    "description": "ATM 1.2.0 Nature's Aura renderer gallery",
                    "pack_format": 48,
                }
            }
        ),
        ...
        function_root / "load.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            "tellraw @a {\"text\":\"Nature's Aura gallery loaded; run "
        ...
        function_root / "teleport.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            f"tp @s {teleport}"
        ),
